function [sampleData, EDI, EDT, EDR, perc95] = runSampling_SI_1_REPEATED(simData,numRuns,~,sampleSize,sampleInterval,tFinal,progress)
...
for i=1:numRuns
    ...
    samplingRound = 1;
    storedVecs = selectedSimData{i,4};
    while (detectionIndicator == 0 && sampleTime <= tFinal)
        ...
        stateVec = storedVecs{sampleIndex}; % Extract vector containing a 1 for every infected plant and a 0 for every healthy plant
        selectVec = stateVec(1:N); % Sample the first N plants from this vector
